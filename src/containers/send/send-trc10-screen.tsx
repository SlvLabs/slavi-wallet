...
const SendTrc10Screen = (props: SendEthScreenProps) => {
  const addressService = useAddressesService();
  const coinPatternService = useCoinPatternService();
  const pattern = coinPatternService.createTrc10Pattern(props.coin, addressService.getGetterDelegate(props.coin));

  return <SendTronBasedScreen coin={props.coin} pattern={pattern} />;
};

export default SendTrc10Screen;
